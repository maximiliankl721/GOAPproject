...
UCollectPurpleToolAction::UCollectPurpleToolAction()
{
	Name_ = "CollectPurpleToolAction";
	Effect_ = TPair<FString, bool>("HasPurpleTool", true);
	Preconditions_ = TMap<FString, bool>();
	Preconditions_.Add(TPair<FString, bool>("ObtainedPurpleTool", true));
	Cost_ = 1;

}

void UCollectPurpleToolAction::Execute()
{
	...
	if (IsValid(AIController) && AgentComp_->GetWorldMemory()->GetWorldState()["ObtainedPurpleTool"] == true) {
...
void UCollectPurpleToolAction::Finished()
{
	auto AIPawn = Cast<APawn>(AgentComp_->GetOwner());
	auto AIController = Cast<AAIController>(AIPawn->GetController());
