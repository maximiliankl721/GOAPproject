...
UEquipPlatinToolAction::UEquipPlatinToolAction()
{
	Name_ = "EquipPlatinToolAction";
	Effect_ = TPair<FString, bool>("PlatinToolEquipped", true);
	Preconditions_ = TMap<FString, bool>();
	Preconditions_.Add(TPair<FString, bool>("HasPlatinTool", true));
	Cost_ = 1;
}

void UEquipPlatinToolAction::Execute()
{
	...
	if (IsValid(AIController) && IsValid(AIChar) && AgentComp_->GetWorldMemory()->GetWorldState()["HasPlatinTool"] == true && !AIChar->GetToolInventory().IsEmpty()) {
		//search for a PlatinTool in the Inventory and equip it
		for (const auto Tool : AIChar->GetToolInventory()) {

			if (Tool->IsA(APlatinTool::StaticClass())) {
...
void UEquipPlatinToolAction::Finished()
{
	auto AIPawn = Cast<APawn>(AgentComp_->GetOwner());
	auto AIController = Cast<AAIController>(AIPawn->GetController());
